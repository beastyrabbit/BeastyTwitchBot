...
def calculate_interest(user: User):
    ...
    # rate is 0.02
    daily_interest_rate = global_settings_Manager.getCommand(
        OfficialCommands.invest
    ).daily_interest_rate
        ...
        messages=[
            {
                "role": "user",
                "content": (
                    "The Narrator should do a very short battle report. like max 3 short sentences"
                    "Include dramatic flair, intense emotions, and heroic descriptions. "
                    "Here's the fight log:\n\n"
                    f"{formatted_fight}"
                ),
            }
        ],
...
def get_text_to_spech(text: str, voice: str = "onyx", hd: bool = False) -> bytes:
    # Text-to-Speech
    if hd:
        modeltype = "tts-1-hd"
    else:
    ...
    AUDIO_PATH = os.path.join(BASE_DIR, "sound", "test_audio.mp3")
    # check file exists and delete it
...
def do_the_cleaning_command(user: User) -> tuple[int, bool]:
    ...
    if global_settings_Manager.check_user_not_in_timeout(user, OfficialCommands.roomba):
        max_value = global_settings_Manager.getCommand(
            OfficialCommands.roomba
        ).current_max_value
        ...
        if get_random == max_value:
            ...
            global_settings_Manager.getCommand(
                OfficialCommands.roomba
